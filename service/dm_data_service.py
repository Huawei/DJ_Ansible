...
class DMRestAPI:
    ...
    def login(self):
        ...
        if validIp is None:
            logging.error(
                'DMRestAPI.login - storage is not accessible: ' + self.ipList.join(
                    ','))
            self.connected = False
            self.cookies = None
        ...
        if response.status_code == 200:
            if body['error']['code'] == 0:
                ...
                logging.error('DMRestAPI.login - %d %s' % (response.status_code,
                                                           json.dumps(body,
                                                                      sort_keys=True,
                                                                      indent=4,
                                                                      ensure_ascii=False)))
...
for storage in VARS['STORAGES']:
    DMAPI[storage['sn']] = DMRestAPI(storage['ipList'], storage['user'],
                                     storage['pswd'], storage['port'])
...
class DMDataService(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def data_detail(self, sn):
        assert self.path.startswith(
            '/deviceManager/rest/' + sn + VARS['DMDATASERVICE']['API'][
                'detail'] + '/')
        ...
        body = response.json()
        resp = []
        if (response.status_code == 200) and (body['error']['code'] == 0) and (
                'data' in body):
            obj = body['data']
                ...
                if (len(name) > 0):
                    resp.append(
                        {'name': '{}{}{}'.format(namePrefix, name, nameSuffix),
                         'value': '{}{}{}'.format(valuePrefix, value,
                                                  valueSuffix)})
            # end body
        # end response
        if len(resp) == 0:
    ...
    def data_search(self, sn):
        ...
        resp = []
        if (response.status_code == 200) and (body['error']['code'] == 0) and (
                'data' in body) and (len(body['data']) > 0):
    ...
    def data_join(self, sn):
        ...
        for join in joins:
            joinResponse = DMAPI[sn].get(
                '%s/%s?filter=%s' % (dmUri, join['obj'], join['filter']));
            joinBody = joinResponse.json()
            if (joinResponse.status_code == 200) and (
                    joinBody['error']['code'] == 0) and (
                    'data' in joinBody) and (len(joinBody['data']) == 1):
                if filterParam is None:
                    filterParam = join['joinAttr'] + '::' + joinBody['data'][0][
                        join['attr']]
                else:
                    filterParam = filterParam + ' and ' + join[
                        'joinAttr'] + '::' + joinBody['data'][0][join['attr']]
        # end joins

        if filterParam is None:
            logging.error(
                'DMDataService.data_join - No matched objects or Not only 1 matched objects')
            self._response([{'name': nameDefault, 'value': valueDefault}])

        # Forward request to DM API
        response = DMAPI[sn].get('%s/%s?range=%s&filter=%s' % (
            dmUri, objType, rangeParam, filterParam));
        body = response.json()
        resp = []
        if (response.status_code == 200) and (body['error']['code'] == 0) and (
                'data' in body) and (len(body['data']) > 0):
    ...
    def data_associate(self, sn):
        ...
        if ('obj' not in params) or ('filter' not in params):
            logging.error(
                'DMDataService.data_associate - No associate obj or filter param')
            self._response([{'name': nameDefault, 'value': valueDefault}])

        resp = []
        joinResponse = DMAPI[sn].get(
            '%s/%s?range=[0-100]&filter=%s' % (dmUri, params['obj'], params['filter']));
        joinBody = joinResponse.json()
        if (joinResponse.status_code == 200) and (
                joinBody['error']['code'] == 0) and ('data' in joinBody) and (
                len(joinBody['data']) == 1):
            ASSOCIATEOBJTYPE = joinBody['data'][0]['TYPE']
            ASSOCIATEOBJID = joinBody['data'][0][objIdAttr]
            # Forward request to DM API
            response = DMAPI[sn].get(
                '%s/%s/associate?range=%s&ASSOCIATEOBJTYPE=%s&ASSOCIATEOBJID=%s' % (
                    dmUri, objType, rangeParam, ASSOCIATEOBJTYPE,
                    ASSOCIATEOBJID));
            body = response.json()
            if (response.status_code == 200) and (
                    body['error']['code'] == 0) and ('data' in body) and (
                    len(body['data']) > 0):
                for obj in body['data']:
                    if nameAttr in obj and valueAttr in obj and matchAttr in obj:
                        if nameField is None:
                        ...
                        if valueField is None:
                            ...
                            value = obj[valueAttr].split(valueSplit)[
                                        valueField][
                                    valueFieldStart:valueFieldEnd]

                        if (len(name) > 0) and (match in obj[matchAttr]):
                            resp.append({'name': '{}{}{}'.format(namePrefix,
                                                                 name,
                                                                 nameSuffix),
                                         'value': '{}{}{}'.format(valuePrefix,
                                                                  value,
            ...
            # end response
                ...
                logging.debug('DMDataService.data_associate - %d %s' % (
        ...
        # end join
            ...
            logging.debug('DMDataService.data_associate - %d %s' % (
                ...
                json.dumps(joinBody, sort_keys=True, indent=4,
                           ensure_ascii=False)))
        if len(resp) == 0:
            self._response([{'name': nameDefault, 'value': valueDefault}])
        else:
...
def parse_args():
    ...
    parser.add_argument('-H', '--host', type=str, required=False,
                        default=VARS['DMDATASERVICE']['host'],
                        help='Listening host or ip address, default: %s' % (
                            VARS['DMDATASERVICE']['host']))
    parser.add_argument('-p', '--port', type=str, required=False,
                        default=VARS['DMDATASERVICE']['port'],
                        help='Listening port, default: %d' % (
                            VARS['DMDATASERVICE']['port']))
    return parser.parse_args()


if __name__ == '__main__':
    args = parse_args()
    log.setup(VARS, "dm_data_service")
    try:

...
class DJRestAPI():
    ...
    def login(self):
        ...
        if response.status_code == 200:
            ...
            logging.error('DJRestAPI.login - %d %s' % (response.status_code,
                                                       json.dumps(body,
                                                                  sort_keys=True,
                                                                  indent=4,
                                                                  ensure_ascii=False)))
...
# load DJ credential
DJAPI = DJRestAPI(VARS['DJ']['host'], VARS['DJ']['user'], VARS['DJ']['pswd'],
                  VARS['DJ']['port'])
...
class DjDataService(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def _response(self, data, code=200):
        ...
        self.end_headers()
        self.wfile.write(json.dumps(data, sort_keys=False, indent=4,
    ...
    def data_enum(self):
        ...
        if enumType not in VARS:
            self._response([{'name': VARS['DEFAULT']['noneName'],
                             'value': VARS['DEFAULT']['noneValue']}])
        ...
        for key in VARS[enumType].keys():
            ...
            if nameAttr == 'key':
                ...
                if nameField is None:
                    ...
                    name = VARS[enumType][key][nameAttr].split(nameSplit)[
                               nameField][nameFieldStart:nameFieldEnd]
            ...
            if valueAttr == 'key':
                ...
                if valueField is None:
                    ...
                    value = VARS[enumType][key][valueAttr].split(valueSplit)[
                                valueField][valueFieldStart:valueFieldEnd]
            ...
            for attr, expect in filterAttrs.items():
                ...
                if (attr not in VARS[enumType][key]) or (
                        VARS[enumType][key][attr] != expect):
                    matched = False
                    break
            # end filter
            ...
            if matched == True and len(name) > 0:
                resp.append(
                    {'name': '{}{}{}'.format(namePrefix, name, nameSuffix),
                     'value': '{}{}{}'.format(valuePrefix, value, valueSuffix)})
        # end keys
    ...
    def data_search(self):
        ...
        if objType not in VARS['INVENTORY']:
            logging.error(
                'DjDataService.data_search - unsupported object type: ' + objType)
            self._response({}, 404)
            return
        ...
        djUri += '&contentSelector=%s' % (
            json.dumps(contentSelector, ensure_ascii=False))
        ...
        if 'changedBefore' in params:
            last_Modified_Before = int(
                (currentTime - int(params['changedBefore'])) * 1000)
            condition['constraint'].append({"logOp": "and",
                                            "simple": {"name": "last_Modified",
                                                       "operator": "less than",
                                                       "value": last_Modified_Before}})

        if 'changedAfter' in params:
            last_Modified_After = int(
                (currentTime - int(params['changedAfter'])) * 1000)
            condition['constraint'].append({"logOp": "and",
                                            "simple": {"name": "last_Modified",
            ...
            for obj in body['objList']:
                if nameAttr in obj and valueAttr in obj:
                    if nameField is None:
                        ...
                        name = obj[nameAttr].split(nameSplit)[nameField][
                               nameFieldStart:nameFieldEnd]
                    ...
                    if valueField is None:
                        ...
                        value = obj[valueAttr].split(valueSplit)[valueField][
                                valueFieldStart:valueFieldEnd]

                    if len(name) > 0 and (
                            (nameUnique == True and name not in names) or (
                            nameUnique == False)) and (
                            (valueUnique == True and value not in values) or (
                            valueUnique == False)):
                        names.append(name)
                        values.append(value)
                        resp.append({'name': '{}{}{}'.format(namePrefix, name,
                                                             nameSuffix),
                                     'value': '{}{}{}'.format(valuePrefix,
                                                              value,
                                                              valueSuffix)})
            # end objList
        # end body
    ...
    def data_join(self):
        ...
        if objType not in VARS['INVENTORY']:
            logging.error(
                'DjDataService.data_join - unsupported object type: ' + objType)
            self._response({}, 404)
            return
        ...
        if 'joins' in params:
            ...
            for join in joins:
                ...
                joinCondition = json.dumps(join['condition'],
                                           ensure_ascii=False)
                joinAttr = join['joinAttr']
                if joinObjType not in VARS['INVENTORY']:
                    logging.error(
                        'DjDataService.data_search - unsupported object type: ' + joinObjType)
                    self._response({}, 404)
                    return
                joinClassName = VARS['INVENTORY'][joinObjType]['className']
                joinUri = '%s/%s?condition=%s' % (
                    VARS['DJSERVICE']['API']['instances'], joinClassName,
                    joinCondition)
                joinResponse = DJAPI.get(joinUri);
                logging.debug('DjDataService.data_join - %d %s' % (
                    joinResponse.status_code, joinUri))
                joinBody = joinResponse.json()
                if (joinResponse.status_code == 200) and (
                        'totalNum' in joinBody) and (joinBody['totalNum'] > 0):
                    joinValues = []
        ...
        if 'relations' in params:
            ...
            for relation in relations:
                ...
                if (relObjType not in VARS['INVENTORY']) or (
                        relObjType not in VARS['INVENTORY'][objType][
                    'relations']):
                    logging.error(
                        'DjDataService.data_search - unsupported object type: ' + relObjType)
                    self._response({}, 404)
                    return
                relClassName = VARS['INVENTORY'][relObjType]['className']
                relationName = \
                    VARS['INVENTORY'][objType]['relations'][relObjType][
                        'relationName']
                isSourceObj = \
                    VARS['INVENTORY'][objType]['relations'][relObjType][
                        'source']
                relCondition = json.dumps(relation['condition'],
                                          ensure_ascii=False)
                relUri = '%s/%s?condition=%s' % (
                    VARS['DJSERVICE']['API']['instances'], relClassName,
                    relCondition)
                relResponse = DJAPI.get(relUri)
                logging.debug('DjDataService.data_join - %d %s' % (
                    relResponse.status_code, relUri))
                relBody = relResponse.json()
                if (relResponse.status_code == 200) and (
                        'totalNum' in relBody) and (relBody['totalNum'] > 0):
                    relValues = []
                    for relObj in relBody['objList']:
                        relValues.append(relObj['id'])
                    if len(relValues) > 0:
                        if isSourceObj == True:
                            condition['relationConstraint'].append(
                                {"logOp": "and", "relationName": relationName,
                                 "sourceInstance": "false", "constraint": [{
                                    "simple": {
                            ...
                            condition['relationConstraint'].append(
                                {"logOp": "and", "relationName": relationName,
                                 "sourceInstance": "true", "constraint": [{
                                    "simple": {
    ...
    def data_split(self):
        ...
        if objType not in VARS['INVENTORY']:
            logging.error(
                'DjDataService.data_split - unsupported object type: ' + objType)
            self._response({}, 404)
            return
        ...
        if ('totalNum' in body) and (body['totalNum'] > 0):
            for obj in body['objList']:
                if valueAttr in obj:
                    ...
                    for value in values:
                        if (len(value) > 0) and (bypass not in value) and (
                                match in value):
                            resp.append({'name': value, 'value': value,
                                         'selected': selected})
                    # end values
                # end valueAttr
            # end objList
